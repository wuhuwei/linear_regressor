...
object LinearRegressor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (false) {
      parseTokens(args(1), parseDict(args(0), FEATURE_THRESHOLD)._1)
      /*
  ...
  def parseTokens(tokensPath: String, tagDict: HashMap[Int, String]) = {
    ...
    try {
      ...
      var wordQueue: List[String] = List[String]();
      ...
      while (true) {
        ...
        if (tokenStr == startReviewTextDelimiter) {
          ...
          if (numReviewsProcessed % BLOCK_SIZE == 0) {
            saveMatrixFile(rowInd, colInd, values, ratings.toList, numReviewsProcessed, tagDict.size)
          ...
          if (tagDict.contains(tokenNum)) { // only count the words that are in our feature set
  ...
  private def saveMatrixFile(rowInd: List[Int], colInd: List[Int], values: List[Int], ratingsList: List[(Int, Int)], numReviewsProcessed: Int, frequencyThreshold: Int): Unit = {
    var counts: SMat = sparse(icol(rowInd), icol(colInd), icol(values))
    counts = counts \ sparse(zeros(counts.nrows, frequencyThreshold - counts.ncols))
